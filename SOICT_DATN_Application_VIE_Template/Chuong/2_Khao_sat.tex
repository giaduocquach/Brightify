...
\begin{tikzpicture}[font=\small,
  ...
  \node[font=\scriptsize, red!60!black, align=center] at (-3.5,-2.7) {dễ gặp cold-start};
  \node[font=\scriptsize, green!45!black, align=center] at (0,-2.7) {miễn nhiễm cold-start\\\textbf{(Brightify)}};
  \node[font=\scriptsize, align=center] at (3.5,-2.7) {kết hợp cả hai};
\end{tikzpicture}
\caption{Phân loại hệ gợi ý: lọc cộng tác phụ thuộc hành vi người dùng nên dễ gặp cold-start; Brightify đi theo hướng dựa trên nội dung}
...
Bài toán cold-start thực ra có ba dạng \cite{schein2002coldstart}: \textit{người dùng mới} (chưa có lịch sử để suy sở thích), \textit{vật phẩm mới} (bài hát mới chưa ai nghe nên lọc cộng tác không có tín hiệu tương tác), và \textit{hệ thống mới} (toàn bộ nền tảng chưa tích lũy dữ liệu hành vi). Lọc cộng tác bất lực với cả ba dạng vì nó hoàn toàn dựa trên ma trận tương tác người dùng--vật phẩm. Ngược lại, gợi ý dựa trên nội dung miễn nhiễm với cold-start \textit{vật phẩm} và \textit{hệ thống}, vì nó chấm điểm dựa trên đặc trưng nội tại của bài hát (âm thanh, lời, cảm xúc) chứ không cần bất kỳ lịch sử nghe nào --- một bài vừa được thêm vào kho có thể được gợi ý ngay sau khi đặc trưng của nó được trích. Đây là lý do nền tảng để đồ án chọn hướng nội dung: nó phù hợp với bối cảnh không có dữ liệu người dùng và một kho nhạc tự thu thập có thể liên tục mở rộng.
...
\hline
PMEmo \cite{zhang2018pmemo} & Nhạc, đoạn chorus (Tây) & 794 bài & V-A động & Kiểm chứng chuyển miền (domain transfer) \\
\hline
...
Bảng~\ref{table:uclist} liệt kê các use case và tác nhân liên quan. Sáu use case chia thành hai nhóm: nhóm phục vụ người nghe (UC01--UC04) và nhóm vận hành dữ liệu (UC05--UC06). Ngoài ra hệ thống còn một số chức năng phụ trợ (tìm kiếm bài hát; phát nhạc và quản lý hàng đợi; nghe crossfade và đài vô tận; duyệt thư viện; xem hướng dẫn và đổi góc nhìn giao diện) --- không phải trọng tâm khoa học nên không đặc tả use case chi tiết, mà chỉ thể hiện qua các yêu cầu chức năng tương ứng ở Bảng~\ref{table:fr}.
...
Trong các yêu cầu trên, FR01 và FR02 là hai chức năng cốt lõi đã nêu ở Chương~\ref{chapter:Introduction}; FR03 mở rộng FR02 cho trường hợp hai màu. FR04--FR06 không phải ``tính năng phụ'' mà là điều kiện để kết quả gợi ý có chất lượng: nếu không loại cover thì danh sách tương tự sẽ trả về chính bài nguồn dưới dạng bản phối khác, còn nếu không đa dạng nghệ sĩ thì danh sách phát dễ bị một nghệ sĩ chiếm trọn. FR07 hiện thực hóa tính ``giải thích được'' của hệ thống. FR08--FR10 giúp hệ thống vận hành được như một phần mềm demo hoàn chỉnh.
...
\begin{tikzpicture}[
  ...
  % merge (center)
  \node[act, text width=5.6cm] (coh) at (0,-8.2) {Chọn cụm nhất quán âm thanh (phạt nghệ sĩ)};
...
\textbf{Tiền điều kiện} & Có quyền vận hành; có danh sách nghệ sĩ hạt giống (seed) và khóa truy cập các nguồn dữ liệu. \\ \hline
...
\hline
NFR06 & Bảo trì / nâng cấp & Thay một thành phần phải vượt cổng không thụt lùi (no-regression gate) trên độ đo cuối & So sánh trước/sau khi thay \\
\hline

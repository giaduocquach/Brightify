...
\begin{table}[h!]
\centering
\caption{Khảo sát khả năng truy vấn theo cảm xúc/màu của các nền tảng nhạc số (đối chiếu định tính)}
\label{table:vnplatforms}
\begin{tabular}{p{3.0cm} p{4.0cm} p{1.9cm} p{1.5cm} p{2.0cm}}
\toprule
\textbf{Nền tảng} & \textbf{Cơ chế gợi ý chính} & \textbf{Truy vấn cảm xúc} & \textbf{Truy vấn màu} & \textbf{Phụ thuộc dữ liệu người dùng} \\
\midrule
Spotify, Apple Music & Lọc cộng tác + biên tập + đặc trưng âm thanh & Playlist tâm trạng biên tập & Không & Cao \\
...
\begin{table}[h!]
\centering
\caption{Các tập dữ liệu cảm xúc nền (gán nhãn người) và vai trò trong đồ án}
\label{table:merdata}
\begin{tabular}{p{2.6cm} p{2.6cm} p{2.0cm} p{2.2cm} p{3.0cm}}
\toprule
...
Tuy nhiên, đánh giá ngoại tuyến có hai cạm bẫy đã được ghi nhận rõ. Thứ nhất là \textit{khoảng cách offline--online}: theo Dacrema và cộng sự \cite{dacrema2019progress}, tương quan giữa các độ đo ngoại tuyến và mức hài lòng thực tế của người dùng chỉ vào khoảng $r \approx 0.28$, nên một cải thiện ngoại tuyến không tự động kéo theo cải thiện trải nghiệm. Thứ hai là vấn đề \textit{baseline yếu}: nhiều công bố báo cáo tiến bộ chỉ vì so với baseline được cấu hình kém, và Dacrema và cộng sự cho thấy không ít phương pháp ``tân tiến'' không vượt nổi các baseline đơn giản nếu các baseline đó được tinh chỉnh tử tế. Hai bài học này định hình trực tiếp quy trình đánh giá của đồ án (Chương~\ref{chapter:Experiment}): luôn đối chiếu với baseline mạnh (kể cả baseline theo độ phổ biến), bổ sung các kiểm chứng \textit{độc lập với nhãn} để giảm phụ thuộc vào một độ đo tự tham chiếu, và nêu thẳng rằng mọi kết quả là độ đo ngoại tuyến chứ chưa phải bằng chứng từ người nghe thật. Như vậy, ràng buộc ``không có người dùng'' không loại bỏ khả năng đánh giá, mà chuyển trọng tâm sang một quy trình ngoại tuyến nhiều tầng, có kiểm định thống kê và tam giác hóa bằng nhiều nguồn độc lập.
...
\label{section:2.3}
Phần này đặc tả các use case quan trọng theo bốn yếu tố: tiền điều kiện, luồng sự kiện chính, luồng thay thế/ngoại lệ và hậu điều kiện. Đặc tả của sáu use case nghiệp vụ chính lần lượt được trình bày trong Bảng~\ref{table:uc01} (UC01), Bảng~\ref{table:uc02} (UC02), Bảng~\ref{table:uc03} (UC03), Bảng~\ref{table:uc04} (UC04), Bảng~\ref{table:uc05} (UC05) và Bảng~\ref{table:uc06} (UC06). Các con số kỹ thuật (cấu trúc dữ liệu trả về, mã lỗi, thời gian phản hồi) được trình bày chi tiết trong Chương~\ref{chapter:Experiment}.

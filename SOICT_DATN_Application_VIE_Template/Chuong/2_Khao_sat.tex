...
\begin{table}[h!]
\centering
\caption{So sánh các công trình màu$\rightarrow$nhạc tiêu biểu với hệ thống đề xuất}
\label{table:colorprior}
\begin{tabular}{p{3.0cm} p{2.4cm} p{3.0cm} p{2.6cm}}
\toprule
...
\begin{itemize}[leftmargin=1.4cm]
    ...
    \item \textbf{Hệ thống xử lý đặc trưng offline} (tác nhân phụ): thành phần tự động trích embedding âm thanh (MuQ), tính tín hiệu cảm xúc từ lời, nhịp độ, độ to và sinh các ma trận biểu diễn. Nó được người vận hành kích hoạt và ghi kết quả trở lại kho đặc trưng để pha phục vụ nạp lên.
\end{itemize}

Việc tách hai tác nhân phụ (kho nhạc và hệ thống xử lý đặc trưng offline) là một lựa chọn thiết kế có chủ đích: nó phản ánh đúng kiến trúc hai pha của hệ thống (Chương~\ref{chapter:Experiment}), trong đó toàn bộ tính toán nặng được dồn về pha ngoại tuyến để pha phục vụ chỉ còn các phép tra cứu và nhân ma trận. Bảng~\ref{table:actors} tóm tắt trách nhiệm chính của từng tác nhân.

\begin{table}[h!]
\centering
\caption{Trách nhiệm chính của các tác nhân}
\label{table:actors}
\begin{tabular}{p{4.0cm} p{3.0cm} p{6.0cm}}
\toprule
\textbf{Tác nhân} & \textbf{Loại} & \textbf{Trách nhiệm chính} \\
\midrule
Người nghe & Chính (người) & Chọn màu/bài, xem và phát playlist, xem cảm xúc bài hát \\
...
\begin{tikzpicture}[
  ...
  % System boundary
  \draw[thick, rounded corners] (3.0,-0.4) rectangle (10.0,10.4);
  \node[anchor=north] at (6.5,10.2) {\textbf{Hệ thống Brightify}};

  % Use cases — serving
  \node[uc] (uc1) at (6.5,8.6) {UC01. Chọn bài hát\\để gợi ý bài tương tự};
...
\label{table:fr}
\begin{tabular}{p{1.0cm} p{7.6cm} p{2.2cm} p{1.8cm}}
\toprule
...
FR02 & Gợi ý playlist từ một màu sắc người dùng chọn & UC02 & Cao \\
...
Trong các yêu cầu trên, FR01 và FR02 là hai chức năng cốt lõi đã nêu ở Chương~\ref{chapter:Introduction}; FR03 mở rộng FR02 cho trường hợp hai màu. FR04--FR06 không phải ``tính năng phụ'' mà là điều kiện để kết quả gợi ý có chất lượng: nếu không loại cover thì danh sách tương tự sẽ trả về chính bài nguồn dưới dạng bản phối khác, còn nếu không đa dạng nghệ sĩ thì playlist dễ bị một nghệ sĩ chiếm trọn. FR07 hiện thực hóa tính ``giải thích được'' của hệ thống. FR08--FR10 bảo đảm hệ thống vận hành được như một sản phẩm hoàn chỉnh.
...
\begin{tikzpicture}[
  ...
  \node[dec] (d) at (0,-1.3) {Số màu?};
  % one-colour branch (left, x=-3.8)
  \node[act] (map1) at (-3.8,-2.9) {Ánh xạ màu $\rightarrow$ V-A};
  \node[act] (cdf) at (-3.8,-4.1) {Đích phân vị (CDF)};
  \node[act] (rbf) at (-3.8,-5.3) {Chấm điểm RBF dị hướng};
  % two-colour branch (right, x=3.8)
  \node[act] (map2) at (3.8,-2.9) {Ánh xạ 2 màu $\rightarrow$ V-A};
  \node[act] (iso) at (3.8,-4.1) {Sinh hành trình (iso-principle)};
  \node[act] (sample) at (3.8,-5.3) {Lấy mẫu điểm dừng};
  % merge (center)
  \node[act] (coh) at (0,-6.7) {Chọn cụm nhất quán âm thanh + MMR};
  \node[act] (ret) at (0,-7.9) {Trả về playlist + ánh xạ màu$\rightarrow$cảm xúc};
  \node[term] (e) at (0,-9.1) {Kết thúc};
...
\label{table:nfr}
\begin{tabular}{p{1.0cm} p{3.2cm} p{4.6cm} p{4.0cm}}
\toprule
...
Trong các yêu cầu trên, NFR06 --- \textit{cổng không thụt lùi} (no-regression gate) --- là một yêu cầu đặc thù và quan trọng của đồ án, nên cần làm rõ. Mỗi khi một thành phần học máy được cân nhắc thay thế (ví dụ đổi mô hình embedding lời, đổi trục biểu diễn âm thanh), thành phần mới chỉ được chấp nhận nếu nó \textit{không làm xấu} độ đo cuối của hệ thống so với cấu hình hiện hành, sau khi đã được tinh chỉnh siêu tham số riêng. Ràng buộc này biến quá trình phát triển thành một chuỗi thay đổi được kiểm soát: không có cải tiến nào được đưa vào chỉ vì ``trông có vẻ tốt hơn trên giấy''. Cơ chế này được vận dụng trực tiếp trong khảo sát so sánh mô hình ở Chương~\ref{chapter:Experiment} và được phân tích như một đóng góp phương pháp luận ở Chương~\ref{chapter:SolutionAndContribution}.

...
\begin{table}[h!]
\centering
\caption{So sánh các mô hình biểu diễn cảm xúc và mức phù hợp với bài toán truy hồi}
\label{table:emotion-models}
\begin{tabular}{p{2.7cm} p{2.0cm} p{3.5cm} p{3.6cm}}
\toprule
...
\begin{table}[h!]
\centering
\caption{Ba cơ sở khoa học dùng cho ánh xạ màu sang tọa độ cảm xúc}
\label{table:color-bases}
\begin{tabular}{p{3.0cm} p{6.6cm} p{2.4cm}}
\toprule
...
\begin{figure}[h!]
\centering
% TODO(sinh viên): thay khối giữ chỗ bằng ảnh thật khi đã tải:
% \includegraphics[width=0.62\textwidth]{Hinhve/iceas_wheel.png}
\fbox{\begin{minipage}[c][4.6cm][c]{0.62\textwidth}\centering\itshape
[\,Hình từ tài liệu --- chèn ảnh\,]\\[3pt]
Bánh xe liên kết màu--cảm xúc (ICEAS)\\[4pt]
\footnotesize\upshape(tải theo hướng dẫn ở Phụ lục; lưu \texttt{Hinhve/iceas\_wheel.png})
\end{minipage}}
\caption{Bánh xe liên kết màu--cảm xúc từ khảo sát ICEAS trên gần ba mươi quốc gia (Nguồn: Jonauskaite và cộng sự, 2020 \cite{jonauskaite2020universal})}
\label{fig:iceas-wheel}
\end{figure}
...
\begin{table}[h!]
\centering
\caption{So sánh hai mô hình học tự giám sát âm nhạc MERT và MuQ}
\label{table:muq-mert}
...
Đồ án lựa chọn MuQ làm trục biểu diễn âm thanh sau một so sánh công bằng cho thấy MuQ \textit{ngang hoặc hơn} MERT --- hòa trên các độ đo trực tiếp và vượt có ý nghĩa thống kê ở phép chuyển giao sang một bộ chấm độc lập (chi tiết ở mục~\ref{subsection:eval-bakeoff}). Biểu diễn của MuQ được dùng cho cả ba vai trò nêu trên, bảo đảm một trục biểu diễn nhất quán cho toàn hệ thống.

\begin{figure}[h!]
\centering
% TODO(sinh viên): thay khối giữ chỗ bằng ảnh thật khi đã tải:
% \includegraphics[width=0.8\textwidth]{Hinhve/muq_arch.png}
\fbox{\begin{minipage}[c][4.6cm][c]{0.8\textwidth}\centering\itshape
[\,Hình từ tài liệu --- chèn ảnh\,]\\[3pt]
Kiến trúc MuQ: phổ Mel $\rightarrow$ lượng tử hóa vector dư (Mel-RVQ) $\rightarrow$ biểu diễn 1024 chiều\\[4pt]
\footnotesize\upshape(tải theo hướng dẫn ở Phụ lục; lưu \texttt{Hinhve/muq\_arch.png})
\end{minipage}}
\caption{Kiến trúc mô hình MuQ với mục tiêu lượng tử hóa vector dư trên phổ Mel (Nguồn: Zhu và cộng sự, 2025 \cite{zhu2025muq})}
\label{fig:muq-arch}
\end{figure}
...
\begin{table}[h!]
\centering
\caption{Bốn tín hiệu văn bản: ba tín hiệu dự đoán Valence tuyệt đối và một tín hiệu so sánh tương đồng lời}
\label{table:text-signals}
\begin{tabular}{p{2.4cm} p{3.1cm} p{2.7cm} p{3.0cm}}
\toprule
...
\centering
\begin{tikzpicture}[font=\small,
   mod/.style={draw, rounded corners, align=center, minimum width=2.4cm, minimum height=0.8cm, fill=black!3},
   fuse/.style={draw, thick, rounded corners, align=center, minimum width=2.9cm, minimum height=1.7cm, fill=blue!8},
   res/.style={draw, rounded corners, align=center, minimum width=2.2cm, minimum height=0.8cm, fill=green!8},
   arr/.style={-{Stealth[length=2mm]}, thick}]
  \node[mod] (a) at (0,1.5) {Âm thanh\\\footnotesize MuQ};
  \node[mod] (l) at (0,0) {Lời\\\footnotesize e5-large + đầu dò};
  \node[mod] (c) at (0,-1.5) {Màu sắc\\\footnotesize Oklab};
  \node[fuse] (f) at (4.0,0) {Tổ hợp đa phương thức\\trên trục\\\textbf{Valence--Arousal}};
  \node[res] (o) at (7.6,0) {Danh sách\\gợi ý};
  \draw[arr] (a) -- (f.west|-a); \draw[arr] (l) -- (f); \draw[arr] (c) -- (f.west|-c);
  \draw[arr] (f) -- node[above, font=\scriptsize]{truy hồi + xếp hạng} (o);
\end{tikzpicture}

...
\label{section:1.3}
Đồ án giải quyết bài toán theo định hướng quy mọi đối tượng về cùng một không gian cảm xúc Valence--Arousal \cite{russell1980circumplex}, kết hợp các mô hình học sâu tự giám sát ở dạng đóng băng (frozen) và đầu dò tuyến tính (linear probe), thay vì tinh chỉnh (fine-tuning) hay dùng dữ liệu người dùng. Lựa chọn này xuất phát từ ràng buộc thực tế (không có dữ liệu cảm xúc nhạc Việt để huấn luyện) và từ bằng chứng cho thấy đầu dò tuyến tính trên đặc trưng đóng băng có thể tổng quát hóa tốt hơn tinh chỉnh khi dữ liệu đích lệch miền \cite{kumar2022finetuning}.

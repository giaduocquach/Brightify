...
\begin{itemize}
  \item \textbf{Ground truth dạng playlist biên tập là thước đo không phù hợp.}
        ...
        $=0.96$, TempoCoherence $=0.84$) làm thước đo chính, thay vì NDCG biên tập.
  \item \textbf{Thiếu một chỉ số chính xác độc lập, không gây tranh cãi.} GT tương
        tự âm nhạc hiện chỉ có $n=52$ hạt giống và một bộ chấm; cần mở rộng (nhiều
        hạt giống hơn, nhiều bộ chấm + hệ số Krippendorff $\alpha$) để có công suất
        thống kê.
...
\begin{itemize}
  \item \textbf{Chỉ số Targeting Error mang tính tự quy chiếu một phần.} Nhãn V-A
        ...
        nên TE thấp ($\approx 0.027$) chỉ là điều kiện cần. Bằng chứng \emph{độc lập}
        mạnh hơn là: tương quan giữa độ sáng/độ bão hoà của màu với BPM thực đo
        (librosa) đạt $\rho = +0.46$ và $+0.66$ (Whiteford 2018), và tương quan

...
1. **E1 — `recommend_by_song`: cắt dư thừa + học trọng số** *(M, CAO NHẤT)*: (a) **gộp cảm xúc về 1**; (b) **bỏ tonal/timbral/rhythmic**, tăng MERT; (c) **học trọng số** (SLSQP, ràng buộc sàn diversity).
   ...
   - 🔜 **E1b — re-optimize config 8-signal MERT (production):** mở rộng `optimize-weights` để x0 = `RECO_SONG_WEIGHTS_MERT` (8-dim) + baseline ENABLE_MERT=True; rồi áp dụng có gate. *Đây mới là bước đưa E1 vào production.*

...
- **Multi-judge (§1.4):** musical GT re-chấm bằng gpt-4o-mini → qwen3↔gpt **κ=0.958** → GT không phải
  single-judge artifact (giải quyết điểm yếu lớn nhất của similar-song).
- Đã sửa: 3 comment/số sai, nối `why` cho similar-song, gỡ `color_hsl` chết (giữ song_emotion_vec),
  đọc AUDIO_BACKBONE runtime. Đã commit `ba587b1` + increment này.
...
### 1.4 Multi-judge validation — musical GT KHÔNG phải single-judge artifact

Lo ngại lớn nhất của similar-song là GT chỉ 1 bộ chấm (qwen3:8b). Re-chấm lại toàn bộ 52 pool
(**n=1048 cặp**) bằng **gpt-4o-mini** (provider khác hẳn, key `.env` `OpenAI_API_KEY` — live) và đo
đồng thuận với điểm qwen3 đã cache:

| Chỉ số đồng thuận qwen3 ↔ gpt-4o-mini | Giá trị |
|---|---|
| Spearman ρ (graded 0–3) | **+0.946** |
| Binary relevance agreement (≥2) | **0.99** |
| Cohen κ | **+0.958** (gần tuyệt đối) |

→ Hai judge độc lập **đồng thuận gần như hoàn toàn** → GT đáng tin, không phải artifact của 1 judge.
Dual-judge scores lưu tại `var/runtime/backtest/ground_truth/similar_musical_gpt_rejudge_v1.json`.

**MuQ vs MERT trên consensus 2-judge** (graded = trung bình qwen3+gpt, mỗi backbone CV-reweight riêng):

| | MuQ | MERT | Δ | CI | |
|---|---|---|---|---|---|
| musical CV-NDCG@10 (consensus) | 0.7351 | 0.7353 | −0.0002 | [−0.021,+0.019] ns | hòa |
| audio-only NDCG@10 (consensus) | 0.7478 | 0.7423 | +0.0055 | [−0.014,+0.025] ns | hòa |

Khớp kết quả §1.1: MuQ ≈ MERT trên metric trực tiếp + MuQ thắng SIG ở GPT-transfer → **MuQ justified**,
và bằng chứng giờ **mạnh hơn nhiều** (GT đã được 2 judge độc lập xác nhận κ=0.958). Giá: ~$0.05, 0 fail.

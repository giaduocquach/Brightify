...
- **[AI]** Hiện chỉ dùng ở `recommend_by_song`. Mở sang: (a) KG content rebuild ✅ (F6), (b) ~~"Audio Radio" thuần chất âm~~ **ĐÃ GỠ (2026-05-30)** — xem dưới, (c) bổ sung tín hiệu context/journey/color (chưa), (d) phát hiện duplicate/cover (chưa).
- **[Nhánh (b) Audio Radio — ĐÃ LÀM RỒI GỠ]:** từng làm `recommend_by_audio` (k-NN MERT) + endpoint + context-menu "🎧 Radio chất âm". **Gỡ surface 2026-05-30** sau kiểm chứng: top-10 chỉ trùng **~1.1/10** với Similar Song (khác ~89%) → *không trùng nhưng "khác ≠ tốt hơn"*, niche + rối UX (2 cửa tìm-tương-tự). Giữ `recommend_by_audio` **dormant** cho option B (núm "Tổng thể⟷Thuần chất âm" trong Similar Song) nếu sau đo được nhu cầu. Chi tiết: `FEATURE_SIGNAL_DESIGN_V11.md` §F-AUDIO.
...
14. **F7**: MERT vào KG ✅ (F6). ~~Audio Radio~~ **đã làm rồi GỠ (2026-05-30)** (trùng Similar 1.1/10 nhưng *khác≠tốt hơn*+niche+rối UX; code dormant). Còn (c) bổ sung tín hiệu context/journey/color + (d) cover/dup-detection *(cần backtest nếu đổi default)*.
